...
export class ListadoAtencionComponent implements OnInit 
{
  ...
  InicializaDatosBusqueda(){
     ...
     this.iniciativas.filterPredicate = (data, filter) => {
      const dataStr = data.codigoSVT + data.numeroIniciativa + data.titulo + data.jefeProyecto.nombres + data.estado.descripcion + data.fechaInicio  + data.fechaFin + data.prioridad.descripcion + data.categoria.descripcion;
